...
for i = 1:Titration_resolution
	...
	R = roots([K1, (Kw - C_a0*K1 + C_b0*K1), (- C_a0*Kw - K1*Kw), -Kw^2]);
	...
	BOH(i,1) =(C_b0*Kw)/(Kw + H(i,1)*K1);
	B(i,1) = (C_b0*H(i,1)*K1)/(Kw + H(i,1)*K1);
	HCl(i,1)=C_a0;
	OH(i,1) = Kw/H(i,1);
	FHCl(i,1)=C_a0;
	FBOH(i,1)=C_b0;
