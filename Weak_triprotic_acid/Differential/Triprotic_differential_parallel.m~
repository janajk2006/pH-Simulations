...
KW = power(10,-14);
K1 = power(10,-3);		%power(10,-3.08);
K2 = power(10,-6); 		%power(10,-4.74);
K3 = power(10,-9); 		%power(10,-5.4);
...
outfile=fopen('Results_parallel','w');
fprintf(outfile,'Acid_vol  Base_vol Tot_vol H3A(i,1) H2A(i,1) HA(i,1) A(i,1) H(i,1) OH(i,1) PH(i,1) FH3A(i,1) FNa(i,1) Beta(i,1)\n');
...
for i = 1:Titration_resolution
	...
	% Compute the concentration of hydrogen ions
		...
		Y_init(i,4)=0;
		Y_init(i,5)=C_b0;
		Y_init(i,6)=power(10,-7);
		Y_init(i,7)=power(10,-7);
...
parfor i = 1:Titration_resolution
	...
	[T,Y] = ode15s(@Triprotic_ODE,[0,Max_time],Y_init(i,:),options,K);
...
for i = 1:Titration_resolution
	...
	fprintf(outfile,'%.12f %.12f %.12f %.12f %.12f %.12f %.12f %.20f %.20f %.12f %.12f %.12f %.12f\n',H3A_vol(i,1), NaOH_vol(i,1), Tot_vol(i,1), H3A(i,1),H2A(i,1),HA(i,1),A(i,1),H(i,1), OH(i,1), PH(i,1),FH3A(i,1),FNa(i,1),Beta(i,1));
