...
Exp_vol_Data=fopen('Data','r');		% NaH2PO4 Na2PO4
exp_data=fscanf(Exp_vol_Data,'%f',[2 inf]); 
fclose(Exp_vol_Data);
exp_data=exp_data';
[data_row,data_col]=size(exp_data);


Plot_x=size(data_row,1);
Plot_y=size(data_row,1);
...
for row_count = 1:data_row

Acid_vol = exp_data(row_count,1); % ml
Base_vol = exp_data(row_count,2); % ml


% Graph settings
Titration_resolution=2; % number of titration points
dx = Base_vol;
x = 0.0:dx:Base_vol;
...
if i==Titration_resolution
	Plot_x(row_count,1)=FNaH2(i,1)/FNa2H(i,1);
	Plot_y(row_count,1)=PH(i,1);
