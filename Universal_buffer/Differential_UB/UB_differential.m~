...
%Takes 1150.66339 sec on 8 cores, 8 GB memory Dell machine 
clear
clc
tic
...
FH3A=size(Titration_resolution,1);	% Sodium phosphate
FH3B=size(Titration_resolution,1);	% Citric acid
FH3C=size(Titration_resolution,1);	% Boric acid
FNa=size(Titration_resolution,1);	% Boric acid
...
for i = 1:Titration_resolution
	...
	% Compute the concentration of hydrogen ions
		...
		Y_init(11,1)=0;
		Y_init(12,1)=0;
		Y_init(13,1)=power(10,-7);
		Y_init(14,1)=power(10,-7);
		Y_init(15,1)=C_n0;
		...
		[T,Y] = ode15s(@UB_ODE,[0,Max_time],Y_init,options,K);
	...
	fprintf(outfile,'%.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.12f %.20f %.20f %.12f %.12f %.12f %.12f %.12f %.12f\n',H3A_vol(i,1),H3B_vol(i,1),H3C_vol(i,1), x(i), Tot_vol(i,1), H3A(i,1), H2A(i,1),HA(i,1),A(i,1),H3B(i,1), H2B(i,1),HB(i,1),B(i,1),H3C(i,1), H2C(i,1),HC(i,1),C(i,1),H(i,1), OH(i,1), PH(i,1),FH3A(i,1),FH3B(i,1),FH3C(i,1),FNa(i,1),Beta(i,1));
